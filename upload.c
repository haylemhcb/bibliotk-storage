...
int main() {
    ...
    *file_name_end = '\0'; // Terminar la cadena
    ...
    /*print_header();*/
    /*printf("<html><body><h1>Archivo Cargado: %s</h1></body></html>", file_path);*/
    free(buffer);

    system("rm -r $HOME/out-html-pdf");
    system("./pdf_to_web ./upload/* >/dev/null");
    system("mkdir basedoc");
    system("mv $HOME/out-html-pdf ./basedoc/$(basename ./upload/*|cut -d '.' -f1)");
    print_header();
    printf("<script>document.location='/';</script>");
    system("rm ./upload/*");
    system("rm -r $HOME/out-html-pdf");
    system("rm -r ./basedoc/doc");
    system("./gen-library");
    system("rm subir.html");
